...
bool Net::connectWiFi(const char* ssid, const char* pass, uint32_t timeout_ms){
  WiFi.mode(WIFI_STA);
...
void Net::printInfo(){
  Serial.printf("[NET] IP=%s  GW=%s  MASK=%s  DNS0=%s  RSSI=%d dBm\n",
...
bool Net::ntpSync(uint32_t timeout_ms){
...
uint16_t Net::httpPing(const char* url, uint16_t timeout_ms){
  if (WiFi.status()!=WL_CONNECTED) return 0;

  if (strstr(url,"ngrok-free.app") || strstr(url,"trycloudflare.com")) {
    if (timeout_ms < 7000) timeout_ms = 7000;
  }
  ...
  if (strncmp(url,"https://",8)==0){
    ...
    WiFiClient client; client.setTimeout((timeout_ms+999)/1000);
    HTTPClient http; http.setTimeout(timeout_ms); http.setFollowRedirects(follow);
